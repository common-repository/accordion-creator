 ...
 * @subpackage Dickensos_Accordion/public
 */
 ...
 */
class Dickensos_Accordion_Public {
	...
	public function enqueue_styles() {
		...
		wp_register_style( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'css/dickensos-accordion-faq-public.css', array(), $this->version, 'all' );
	...
	public function enqueue_scripts() {
		...
		wp_register_script( $this->plugin_name    , plugin_dir_url(__FILE__) . 'js/dickensos-accordion-faq-public.js', array( 'jquery' ), $this->version, false );
	...
	public function get_display_faq( $args = array() ) {
		...
		if( isset( $args['category'] ) && $args['category'] != -1 ) {
			...
			$dickensos_terms            = get_terms('faq_category',
				array(
		...
		if ( count( $dickensos_terms ) > 0 ) {
			foreach ( $dickensos_terms as $term ) {
				$dickensos_terms_questions[ $term->term_id ] = get_posts( array_merge( $qry_args,
					array('tax_query'     =>
						array(
							array(
								'taxonomy'  => 'faq_category',
			...
			$pafa_faq_data = array(
				'dispaly_terms' => TRUE,
				'terms'         => $dickensos_terms,
				'questions'     => $dickensos_terms_questions,
				'template'      => $args['template'],
			);
		} else {

			$dickensos_question = get_posts($qry_args);

			$pafa_faq_data = array(
				'dispaly_terms' => FALSE,
				'questions'     => $dickensos_question,
				'template'      => $args['template'],
			);
		}
		...
		$default_filename           = plugin_dir_path( __FILE__ ) . "partials/dickensos-accordion-faq-public-list.php";
		$theme_default_filename     = get_stylesheet_directory() . "/dickensos-accordion-faq-public-list.php";

		$default_template_filename  = plugin_dir_path( __FILE__ ) . "partials/dickensos-accordion-faq-public-{$args['template']}.php";
		$theme_template_filename    = get_stylesheet_directory() . "/dickensos-accordion-faq-public-{$args['template']}.php";
